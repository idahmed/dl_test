...
def test_nomenclature_delete1(page):
    ...
    assert row.is_visible(), "The row containing text 'Test_nomen' is not found"
    ...
    assert page.locator('h2:has-text("Nomenclatures")').is_visible(), "'Nomenclatures' header is not visible"


def test_nomenclature_delete2(page):
    page.goto(BASE_URL + "login/")
    page.locator("#djHideToolBarButton").click()
    assert page.title() == "Login"
    page.fill("#email", "admin@admin.com")
    ...
    assert page.url == f"{BASE_URL}", "Redirection after login failed"
    page.goto(BASE_URL + "nomenclatures/")
    row = page.locator('tr', has_text="Test_nomen Test2")
    assert row.is_visible(), "The row containing text 'Test_nomen' is not found"
    delete_button = row.locator('a[data-bs-toggle="modal"]')
    assert delete_button.is_visible(), "Delete button in the row is not found"
    delete_button.click()
